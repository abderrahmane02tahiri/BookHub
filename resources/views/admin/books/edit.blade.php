    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.books.update", [$book->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('genres') ? 'is-invalid' : '' }}" name="genres_id" id="genres_id" required>
                    @foreach($genres as $id => $entry)
                        <option value="{{ $id }}" {{ (old('genres_id') ? old('genres_id') : $book->genres->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('notes_and_description') ? 'is-invalid' : '' }}" name="notes_and_description" id="notes_and_description">{!! old('notes_and_description', $book->notes_and_description) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('language') ? 'is-invalid' : '' }}" name="language" id="language" required>
                    ...
                    @foreach(App\Models\Book::LANGUAGE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('language', $book->language) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status_id" id="status_id" required>
                    @foreach($statuses as $id => $entry)
                        <option value="{{ $id }}" {{ (old('status_id') ? old('status_id') : $book->status->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
